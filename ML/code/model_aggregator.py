...
def foolsgold(this_delta, summed_deltas, sig_features_idx, iter, model, topk_prop=0, importance=False,
              importanceHard=False, clip=0):
...
def foolsgold2(this_delta, summed_deltas, sig_features_idx, iter, model, wv, topk_prop=0
               , importance=False, importanceHard=False, clip=0):
    # Take all the features of sig_features_idx for each clients
    sd = summed_deltas.copy()
    sig_filtered_deltas = np.take(sd, sig_features_idx, axis=1)
    if importance or importanceHard:
    ...
    cs = smp.cosine_similarity(sig_filtered_deltas) - np.eye(n)
    # Pardoning: reweight by the max value seen
    cs_avg = np.sum(cs, axis=1) / (n - 1)
    # cs_avg = np.min(cs, axis=1)
    # wv = np.ndarray(shape=(n, ))
    baseline_idx = np.argmax(cs_avg)
    for i in range(n):
        wv[i] = (1 - cs[baseline_idx][i]) / np.sum(wv)
